...
(function () {
    ...
    */
    function parseAsanaDate(dateText, today = new Date(), prevDate = null) {
        if (weeks.includes(dateText)) {
            ...
            date.setDate(today.getDate() + week - dayOfWeek + (week < dayOfWeek ? 7 : 0));
            return date;
        }
    ...
    */
    function convertDueDateToDate() {
        ...
        let prevDate = null;
        document.querySelectorAll("span.DueDate-noWrapSegment:not(.converted), [data-testid='DueDate']:not(.convertedDateFormat)").forEach((span) => {
            const isRange = span.textContent.startsWith(" – ");
            const date = parseAsanaDate(span.textContent, today, isRange ? prevDate : null);
